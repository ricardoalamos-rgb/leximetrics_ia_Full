...
import sys
import os
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))) # Add app root to path
import requests
import json
import chromadb
...
# Configuration - should match app.config but hardcoded for script simplicity/independence
CHROMA_PERSIST_DIR = "/app/chroma_db"
EMBEDDING_MODEL_NAME = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"
TEMPLATES_DIR = "/app/data/templates"

# List of URLs to download (Same as download_templates.py)
URLS = [
...
def download_file(file_id, dest_path):
    ...
    try:
        ...
        if response.status_code == 200:
            filename = f"{file_id}.pdf" # Default to pdf for indexing
...
def main():
    ...
    for url in URLS:
        ...
        if fid: download_file(fid, TEMPLATES_DIR)
    ...
    try:
        ...
        print("✅ ChromaDB Collection 'libros' ready.")
    ...
    for f in files:
        if f.startswith(".") or f.endswith(".json"): continue
        ...
        try:
            if f.lower().endswith('.pdf'):
                ...
                # Basic read for others (unlikely standard reader for .doc without libreoffice)
                # Skip .doc/docx for now in RAG content unless simple text
                print(f"      ⚠️ Skipping non-PDF text extraction for now: {f}")
                continue
        ...
        if chunks:
            ...
            try:
                ...
                print(f"      ❌ Error embedding/upserting: {e}")
        
    print(f"✨ Finished. Ingested {count} books ({chunks_total} chunks).")
